import FlagAfghanistan from './svg/flag_afghanistan.svg';
import FlagAlbania from './svg/flag_albania.svg';
import FlagAlgeria from './svg/flag_algeria.svg';
import FlagAngola from './svg/flag_angola.svg';
import FlagAntiguaAndBarbuda from './svg/flag_antiguaandbarbuda.svg';
import FlagArgentina from './svg/flag_argentina.svg';
import FlagArmenia from './svg/flag_armenia.svg';
import FlagAustralia from './svg/flag_australia.svg';
import FlagAustria from './svg/flag_austria.svg';
import FlagAzerbaijan from './svg/flag_azerbaijan.svg';
import FlagBahamas from './svg/flag_bahamas.svg';
import FlagBahrain from './svg/flag_bahrain.svg';
import FlagBangladesh from './svg/flag_bangladesh.svg';
import FlagBarbados from './svg/flag_barbados.svg';
import FlagBelarus from './svg/flag_belarus.svg';
import FlagBelgium from './svg/flag_belgium.svg';
import FlagBelize from './svg/flag_belize.svg';
import FlagBenin from './svg/flag_benin.svg';
import FlagBhutan from './svg/flag_bhutan.svg';
import FlagBoliva from './svg/flag_boliva.svg';
import FlagBosnia from './svg/flag_bosnia.svg';
import FlagBotswana from './svg/flag_botswana.svg';
import FlagBrazil from './svg/flag_brazil.svg';
import FlagBViginIslands from './svg/flag_britishvirginislands.svg';
import FlagBrunei from './svg/flag_brunei.svg';
import FlagBulgary from './svg/flag_bulgary.svg';
import FlagBurkinaFaso from './svg/flag_burkinafaso.svg';
import FlagBurundi from './svg/flag_burundi.svg';
import FlagCaboVerde from './svg/flag_caboverde.svg';
import FlagCambodia from './svg/flag_cambodia.svg';
import FlagCameroon from './svg/flag_cameroon.svg';
import FlagCanada from './svg/flag_canada.svg';
import FlagCAfricanRepublic from './svg/flag_centralafricanrepublic.svg';
import FlagChad from './svg/flag_chad.svg';
import FlagChile from './svg/flag_chile.svg';
import FlagChina from './svg/flag_china.svg';
import FlagColombia from './svg/flag_colombia.svg';
import FlagComoros from './svg/flag_comoros.svg';
import FlagCostaRica from './svg/flag_costarica.svg';
import FlagCotedIvoire from './svg/flag_cotedivoire.svg';
import FlagCroatia from './svg/flag_croatia.svg';
import FlagCuba from './svg/flag_cuba.svg';
import FlagCyprus from './svg/flag_cyprus.svg';
import FlagCzechRepublic from './svg/flag_czechrepublic.svg';
import FlagDRCongo from './svg/flag_democraticrepublicofthecongo.svg';
import FlagDenmark from './svg/flag_denmark.svg';
import FlagDjibouti from './svg/flag_djibouti.svg';
import FlagDominicanRepublic from './svg/flag_dominicanrepublic.svg';
import FlagEcuador from './svg/flag_ecuador.svg';
import FlagEgypt from './svg/flag_egypt.svg';
import FlagElSalvador from './svg/flag_elsalvador.svg';
import FlagEquatorialGuinea from './svg/flag_equatorialguinea.svg';
import FlagEritrea from './svg/flag_eritrea.svg';
import FlagEstonia from './svg/flag_estonia.svg';
import FlagEswatini from './svg/flag_eswatini.svg';
import FlagEthiopia from './svg/flag_ethiopia.svg';
import FlagFiji from './svg/flag_fiji.svg';
import FlagFinland from './svg/flag_finland.svg';
import FlagFrance from './svg/flag_france.svg';
import FlagGabon from './svg/flag_gabon.svg';
import FlagGambia from './svg/flag_gambia.svg';
import FlagGeorgia from './svg/flag_georgia.svg';
import FlagGermany from './svg/flag_germany.svg';
import FlagGhana from './svg/flag_ghana.svg';
import FlagGibraltar from './svg/flag_gibraltar.svg';
import FlagGreece from './svg/flag_greece.svg';
import FlagGrenada from './svg/flag_grenada.svg';
import FlagGuatemala from './svg/flag_guatemala.svg';
import FlagGuinea from './svg/flag_guinea.svg';
import FlagGuineaBissau from './svg/flag_guineabissau.svg';
import FlagGuyana from './svg/flag_guyana.svg';
import FlagHaiti from './svg/flag_haiti.svg';
import FlagHonduras from './svg/flag_honduras.svg';
import FlagHongKong from './svg/flag_hongkong.svg';
import FlagHungary from './svg/flag_hungary.svg';
import FlagIceland from './svg/flag_iceland.svg';
import FlagIndia from './svg/flag_india.svg';
import FlagIndonesia from './svg/flag_indonesia.svg';
import FlagIran from './svg/flag_iran.svg';
import FlagIraq from './svg/flag_iraq.svg';
import FlagIreland from './svg/flag_ireland.svg';
import FlagIsrael from './svg/flag_israel.svg';
import FlagItaly from './svg/flag_italy.svg';
import FlagJamaica from './svg/flag_jamaica.svg';
import FlagJapan from './svg/flag_japan.svg';
import FlagJordan from './svg/flag_jordan.svg';
import FlagKazakhstan from './svg/flag_kazakhstan.svg';
import FlagKenya from './svg/flag_kenya.svg';
import FlagKiribati from './svg/flag_kiribati.svg';
import FlagKuwait from './svg/flag_kuwait.svg';
import FlagKyrgyzstan from './svg/flag_kyrgyzstan.svg';
import FlagLaos from './svg/flag_laos.svg';
import FlagLatvia from './svg/flag_latvia.svg';
import FlagLebanon from './svg/flag_lebanon.svg';
import FlagLesotho from './svg/flag_lesotho.svg';
import FlagLiberia from './svg/flag_liberia.svg';
import FlagLibya from './svg/flag_libya.svg';
import FlagLithuania from './svg/flag_lithuania.svg';
import FlagLuxembourg from './svg/flag_luxembourg.svg';
import FlagMadagascar from './svg/flag_madagascar.svg';
import FlagMalasya from './svg/flag_malasya.svg';
import FlagMalawi from './svg/flag_malawi.svg';
import FlagMaldives from './svg/flag_maldives.svg';
import FlagMali from './svg/flag_mali.svg';
import FlagMalta from './svg/flag_malta.svg';
import FlagMauritania from './svg/flag_mauritania.svg';
import FlagMauritius from './svg/flag_mauritius.svg';
import FlagMexico from './svg/flag_mexico.svg';
import FlagMicronesia from './svg/flag_micronesia.svg';
import FlagMoldova from './svg/flag_moldova.svg';
import FlagMongolia from './svg/flag_mongolia.svg';
import FlagMontenegro from './svg/flag_montenegro.svg';
import FlagMorocco from './svg/flag_morocco.svg';
import FlagMozambique from './svg/flag_mozambique.svg';
import FlagMyanmar from './svg/flag_myanmar.svg';
import FlagNamibia from './svg/flag_namibia.svg';
import FlagNepal from './svg/flag_nepal.svg';
import FlagNetherlands from './svg/flag_netherlands.svg';
import FlagNewZealeand from './svg/flag_newzealand.svg';
import FlagNicaragua from './svg/flag_nicaragua.svg';
import FlagNiger from './svg/flag_niger.svg';
import FlagNigeria from './svg/flag_nigeria.svg';
import FlagNorthKorea from './svg/flag_northkorea.svg';
import FlagNorthMacedonia from './svg/flag_northmacedonia.svg';
import FlagNorway from './svg/flag_norway.svg';
import FlagOman from './svg/flag_oman.svg';
import FlagPakistan from './svg/flag_pakistan.svg';
import FlagPanama from './svg/flag_panama.svg';
import FlagPapuaNewGuinea from './svg/flag_papuanewguinea.svg';
import FlagParaguay from './svg/flag_paraguay.svg';
import FlagPeru from './svg/flag_peru.svg';
import FlagPhilippines from './svg/flag_philippines.svg';
import FlagPoland from './svg/flag_poland.svg';
import FlagPortugal from './svg/flag_portugal.svg';
import FlagPuertoRico from './svg/flag_puertorico.svg';
import FlagQatar from './svg/flag_qatar.svg';
import FlagRCongo from './svg/flag_republicofthecongo.svg';
import FlagRomania from './svg/flag_romania.svg';
import FlagRussia from './svg/flag_russia.svg';
import FlagRwanda from './svg/flag_rwanda.svg';
import FlagStLucia from './svg/flag_saintlucia.svg';
import FlagStVincentGrenadines from './svg/flag_saintvincentandthegrenadines.svg';
import FlagSamoa from './svg/flag_samoa.svg';
import FlagSTomePrincipe from './svg/flag_saotomeandprincipe.svg';
import FlagSaudiArabia from './svg/flag_saudiarabia.svg';
import FlagSenegal from './svg/flag_senegal.svg';
import FlagSerbia from './svg/flag_serbia.svg';
import FlagSeychelles from './svg/flag_seychelles.svg';
import FlagSierraLeone from './svg/flag_sierraleone.svg';
import FlagSingapore from './svg/flag_singapore.svg';
import FlagSlovakia from './svg/flag_slovakia.svg';
import FlagSlovenia from './svg/flag_slovenia.svg';
import FlagSolomonIslands from './svg/flag_solomonisland.svg';
import FlagSomalia from './svg/flag_somalia.svg';
import FlagSouthAfrica from './svg/flag_southafrica.svg';
import FlagSouthKorea from './svg/flag_southkorea.svg';
import FlagSouthSudan from './svg/flag_southsudan.svg';
import FlagSpain from './svg/flag_spain.svg';
import FlagSriLanka from './svg/flag_srilanka.svg';
import FlagSudan from './svg/flag_sudan.svg';
import FlagSuriname from './svg/flag_suriname.svg';
import FlagSweden from './svg/flag_sweden.svg';
import FlagSwitzerland from './svg/flag_switzerland.svg';
import FlagSyria from './svg/flag_syria.svg';
import FlagTaiwan from './svg/flag_taiwan.svg';
import FlagTajikistan from './svg/flag_tajikistan.svg';
import FlagTanzania from './svg/flag_tanzania.svg';
import FlagThailand from './svg/flag_thailand.svg';
import FlagTimorLeste from './svg/flag_timorleste.svg';
import FlagTogo from './svg/flag_togo.svg';
import FlagTonga from './svg/flag_tonga.svg';
import FlagTrinidadAndTobago from './svg/flag_trinidadandtobago.svg';
import FlagTunisia from './svg/flag_tunisia.svg';
import FlagTurkey from './svg/flag_turkey.svg';
import FlagTurkmenistan from './svg/flag_turkmenistan.svg';
import FlagUK from './svg/flag_uk.svg';
import FlagUnitedArabEmirates from './svg/flag_unitedarabemirates.svg';
import FlagUS from './svg/flag_us.svg';
import FlagUganda from './svg/flag_uganda.svg';
import FlagUkraine from './svg/flag_ukraine.svg';
import FlagUruguay from './svg/flag_uruguay.svg';
import FlagUzbekistan from './svg/flag_uzbekistan.svg';
import FlagVanuatu from './svg/flag_vanuatu.svg';
import FlagVenezuela from './svg/flag_venezuela.svg';
import FlagVietnam from './svg/flag_vietnam.svg';
import FlagYemen from './svg/flag_yemen.svg';
import FlagZambia from './svg/flag_zambia.svg';
import FlagZimbabwe from './svg/flag_zimbabwe.svg';

export {
  FlagAfghanistan,
  FlagAlbania,
  FlagAlgeria,
  FlagAngola,
  FlagAntiguaAndBarbuda,
  FlagArgentina,
  FlagArmenia,
  FlagAustralia,
  FlagAustria,
  FlagAzerbaijan,
  FlagBahamas,
  FlagBahrain,
  FlagBangladesh,
  FlagBarbados,
  FlagBelarus,
  FlagBelgium,
  FlagBelize,
  FlagBenin,
  FlagBhutan,
  FlagBoliva,
  FlagBosnia,
  FlagBotswana,
  FlagBrazil,
  FlagBViginIslands,
  FlagBrunei,
  FlagBulgary,
  FlagBurkinaFaso,
  FlagBurundi,
  FlagCaboVerde,
  FlagCambodia,
  FlagCameroon,
  FlagCanada,
  FlagCAfricanRepublic,
  FlagChad,
  FlagChile,
  FlagChina,
  FlagColombia,
  FlagComoros,
  FlagCostaRica,
  FlagCotedIvoire,
  FlagCroatia,
  FlagCuba,
  FlagCyprus,
  FlagCzechRepublic,
  FlagDenmark,
  FlagDRCongo,
  FlagDjibouti,
  FlagDominicanRepublic,
  FlagEcuador,
  FlagEgypt,
  FlagElSalvador,
  FlagEquatorialGuinea,
  FlagEritrea,
  FlagEstonia,
  FlagEswatini,
  FlagEthiopia,
  FlagFiji,
  FlagFinland,
  FlagFrance,
  FlagGabon,
  FlagGambia,
  FlagGeorgia,
  FlagGermany,
  FlagGhana,
  FlagGibraltar,
  FlagGreece,
  FlagGrenada,
  FlagGuatemala,
  FlagGuinea,
  FlagGuineaBissau,
  FlagGuyana,
  FlagHaiti,
  FlagHonduras,
  FlagHongKong,
  FlagHungary,
  FlagIceland,
  FlagIndia,
  FlagIndonesia,
  FlagIran,
  FlagIraq,
  FlagIreland,
  FlagIsrael,
  FlagItaly,
  FlagJamaica,
  FlagJapan,
  FlagJordan,
  FlagKazakhstan,
  FlagKenya,
  FlagKiribati,
  FlagKuwait,
  FlagKyrgyzstan,
  FlagLaos,
  FlagLatvia,
  FlagLebanon,
  FlagLesotho,
  FlagLiberia,
  FlagLibya,
  FlagLithuania,
  FlagLuxembourg,
  FlagMadagascar,
  FlagMalasya,
  FlagMalawi,
  FlagMaldives,
  FlagMali,
  FlagMalta,
  FlagMauritania,
  FlagMauritius,
  FlagMexico,
  FlagMicronesia,
  FlagMoldova,
  FlagMongolia,
  FlagMontenegro,
  FlagMorocco,
  FlagMozambique,
  FlagMyanmar,
  FlagNamibia,
  FlagNepal,
  FlagNetherlands,
  FlagNewZealeand,
  FlagNicaragua,
  FlagNiger,
  FlagNigeria,
  FlagNorthKorea,
  FlagNorthMacedonia,
  FlagNorway,
  FlagOman,
  FlagPakistan,
  FlagPanama,
  FlagPapuaNewGuinea,
  FlagParaguay,
  FlagPeru,
  FlagPhilippines,
  FlagPoland,
  FlagPortugal,
  FlagPuertoRico,
  FlagQatar,
  FlagRCongo,
  FlagRomania,
  FlagRussia,
  FlagRwanda,
  FlagStLucia,
  FlagStVincentGrenadines,
  FlagSamoa,
  FlagSTomePrincipe,
  FlagSaudiArabia,
  FlagSenegal,
  FlagSerbia,
  FlagSeychelles,
  FlagSierraLeone,
  FlagSingapore,
  FlagSlovakia,
  FlagSlovenia,
  FlagSolomonIslands,
  FlagSomalia,
  FlagSouthAfrica,
  FlagSouthKorea,
  FlagSouthSudan,
  FlagSpain,
  FlagSriLanka,
  FlagSudan,
  FlagSuriname,
  FlagSweden,
  FlagSwitzerland,
  FlagSyria,
  FlagTaiwan,
  FlagTajikistan,
  FlagTanzania,
  FlagThailand,
  FlagTimorLeste,
  FlagTogo,
  FlagTonga,
  FlagTrinidadAndTobago,
  FlagTunisia,
  FlagTurkey,
  FlagTurkmenistan,
  FlagUK,
  FlagUnitedArabEmirates,
  FlagUS,
  FlagUganda,
  FlagUkraine,
  FlagUruguay,
  FlagUzbekistan,
  FlagVanuatu,
  FlagVenezuela,
  FlagVietnam,
  FlagYemen,
  FlagZambia,
  FlagZimbabwe,
};
