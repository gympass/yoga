...
describe('Android', () => {
  ...
  describe('fontWeight', () => {
    it('Should return values for fontWeight prop', () => {
      const expectedRegularFontWeight = css({ fontFamily: `${baseFont.family}` });
      const expectedSemiBoldFontWeight = css({ fontFamily: `${baseFont.family}-${fontWeights.semiBold}` });
      const expectedBlackFontWeight = css({ fontFamily: `${baseFont.family}-${fontWeights.black}` });
