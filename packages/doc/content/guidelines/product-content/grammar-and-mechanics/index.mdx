...
for BR, ES, AR, CL, MX, GE, FR, IT, NL, PT. For the other countries (US, UK, IE)
, use the dot.
...
- The UK and Ireland are an exception to this rule, respecting the same rule as the US, including the rule for decimals. Example: €62.99

If you don't have enough space, we can use the reduced version, with only the symbol.
...
separation of each country. And use a country and area code if your reader is in another country.
